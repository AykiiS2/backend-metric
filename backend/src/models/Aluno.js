...
export class Aluno extends BaseModel {
  ...
  async update(id, data) {
    try {
      ...
      if (data.nome_aluno !== undefined) {
        updateData.nome_aluno = data.nome_aluno;
      }

      if (data.rm !== undefined) {
        updateData.rm = data.rm;
      }
      ...
      if (data.id_turma !== undefined) {
        updateData.id_turma = data.id_turma;
      }
  ...
  async delete(id) {
    try {
      ...
      if (error) {
        throw new AppError(
          `Erro ao deletar aluno: ${error.message}`,
          500
        );
      }
    } catch (error) {
  ...
  async getRanking() {
    try {
       ...
       * 1. Busca o ranking.
       *
       * O id_aluno pode ser UUID/hash/string.
       * Em nenhum momento convertemos ele para número.
       */
      const {
        data: rankingData,
        error: rankingError
      ...
      /*
       * 2. Extrai os hashes/IDs dos alunos presentes
       * no ranking.
       */
      const alunoIds = [
        ...new Set(
      ...
      /*
       * 3. Busca o RM REAL na tabela alunos.
       *
       * Não estamos colocando RM em ranking_alunos.
       */
      const {
        data: alunosData,
        error: alunosError
       ...
       * - RM
       *
       * O RM continua vindo da tabela alunos.
       */
      return rankingData.map(
        (item, index) => ({
          ...item,
  ...
  async getRankingByTurma(turmaId) {
    try {
      /*
       * Reutilizamos getRanking() para que
       * o RM também esteja presente.
       */
  ...
  async updatePontuacao(id) {
    try {
      ...
      throw new AppError(
        `Erro ao atualizar pontuação: ${error.message}`,
        400
      );
    }
  }
  ...
  async validateAluno(rm, senha) {
    try {
      const {
        ...
        .from('alunos')
        .select(
          'id_aluno, rm, nome_aluno, senha, id_escola, id_turma, pontuacao'
        )
        .eq('rm', rm)
        .single();
      ...
      throw new AppError(
        `Erro ao validar aluno: ${error.message}`,
        400
      );
    }
  }
